...
import com.paquete.xml.school.StudentAddressRequest;
import com.paquete.xml.school.StudentAddressResponse;
...
public class StudentEndpoint 
{
   ...
   //CADA VEZ QUE ESTA APLICACION RECIVE UN "StudentAddressRequest", SE LANZA ESTE METODO
   @PayloadRoot(namespace = NAMESPACE_URI, localPart = "StudentAddressRequest")
   @ResponsePayload //DEVUELVE UN StudentDetailsResponse
   public StudentAddressResponse getStudent(@RequestPayload StudentAddressRequest request) {
	   StudentAddressResponse response = new StudentAddressResponse();
       response.setAddress(StudentRepository.findStudent(request.getName()).getAddress());
       return response;
   }
}