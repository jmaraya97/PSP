...
/**
 * Clase que controla el uso y peticiones de Proyecto
 * @author usuario
 *
 */
@RestController
public class ProyectoServiceController {
	...
	static {
		...
		d1.setId("1");// se establece el id
		d1.setNombre("Proyecto SOAP");// se establece el nombre
		mapProyectos.put(d1.getId(), d1);// se agrega al objeto dentro del map con los datos indicados
	...
	public ResponseEntity<Object> createDepartamento(@RequestBody Proyecto proy) {
		if (proy.getId() != null && mapProyectos.get(proy.getId()) == null) {
			...
			return new ResponseEntity<>("Proyecto no se creó porque ya existe o la información no es válida", HttpStatus.CREATED);
		}
	}
	...
	public ResponseEntity<Object> getDepartamento() {
		if (mapProyectos.isEmpty()) {
			return new ResponseEntity<>("No hay proyectos", HttpStatus.OK);
		} else {
